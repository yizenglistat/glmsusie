...
#' 
#' 1. Configures data generation with controlled correlation structure between predictors
#' 2. Sets up method-specific parameters for GLMCS and (optionally) SuSiE
#' 3. Runs multiple simulation replicates and collects performance metrics
#' 4. Summarizes results on coefficient recovery and confidence set construction
#' 
...
#'
#' @param n_sims Integer. Number of simulation replicates. Default: 50.
#' @param n Integer. Number of observations per simulation. Default: 600.
#' @param p Integer. Number of predictors. Default: 2 (minimum for Example 1).
#' @param L Integer. Number of single-effect components. Default: 10.
#' @param family GLM family object or "cox". Default: gaussian().
#' @param rho Numeric in (0,1). Correlation coefficient for the two-predictor scenario. 
#'   Default: 0.9.
#' @param intercept Numeric. True intercept for data generation. Default: -1.
#' @param dispersion Numeric. Dispersion parameter for GLMs. Default: 9.
#' @param censoring_rate Numeric in [0,1). Censoring rate for Cox models. Default: 0.3.
#' @param coverage Numeric in (0,1). Confidence set coverage level. Default: 0.95.
#' @param methods Character vector. Methods to benchmark. Default: c("glmcs", "susie").
#' @param standardize Logical. Whether to standardize predictors. Default: TRUE.
#' @param ties String. Cox tie-handling method: "efron" or "breslow". Default: "efron".
#' @param algorithm String. Update strategy: "greedy" or "shuffle". Default: "greedy".
#' @param max_iter Integer. Maximum iterations for optimization. Default: 100.
#' @param step_size Numeric. Step size for coordinate ascent. Default: 1.0.
#' @param tol Numeric. Convergence tolerance. Default: 1.
#' @param min_abs_corr Numeric. Minimum absolute correlation for purity filtering. 
#'   Default: 0.0.
...
#' @param save_freq Integer. How often to save results. Default: 50.
...
run_ex1 <- function(
  ...
  if (p < 2 && settings == "ex1") {
    stop("Example 1 requires at least 2 predictors (p >= 2)")
  }
  
  if (!is.numeric(rho) || rho <= 0 || rho >= 1) {
    warning("rho must be in (0,1). Using default value 0.9.")
    rho <- 0.9
  }
  ...
  tryCatch({
    ...
    stop("Error in benchmark execution: ", e$message)
  })
}

# run_ex1(n_sims=1000,n=10000, family=binomial(),intercept=0, algorithm='greedy',tol=1, max_iter=1000, seed=NULL)
