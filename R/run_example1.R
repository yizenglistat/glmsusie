#' Run Example 1 Benchmark Simulation
#'
#' @description
#' Runs a comprehensive benchmark simulation for Example 1 settings, comparing 
...
#' settings for the "ex1" simulation scenario.
#'
#' @details
...
#'    - Runtime performance
#'
#' The "ex1" setting represents a specific simulation scenario with a minimum of 2 predictors
#' and predefined sparsity structure designed to test confidence set construction in 
#' simple but potentially correlated settings.
#'
...
#' @param p Integer. Number of predictors (must be >= 2 for ex1). Default: 2.
...
#' @param algorithm Character. Algorithm to use: "greedy", "shuffle", or "cyclic". Default: "shuffle".
...
#' results <- run_ex1()
#' 
#' # Run with more predictors
#' results_more <- run_ex1(
#'   p = 10,
#'   n_sims = 30
#' )
#' 
...
#' }
#'
#' @seealso \code{\link{benchmark}}, \code{\link{simulate}}, \code{\link{run_ex2}}
...
run_ex1 <- function(
  ...
  validate_inputs <- function() {
    ...
    if (!algorithm %in% c("greedy", "shuffle", "cyclic")) {
      warning("Parameter 'algorithm' must be either 'greedy', 'shuffle', 'cyclic'. Using default 'shuffle'.")
      algorithm <- "shuffle"
    }
  ...
  # Add run_ex1 specific information to results
  results$run_ex1_settings <- list(
    n              = n,
    p              = p,
    L              = L,
    family         = family,
  ...
  if (verbose) {
    message(sprintf("run_ex1 completed in %.2f minutes.", as.numeric(total_time)))
