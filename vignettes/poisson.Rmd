...
We generate $n=1000$ observations and $p=100$ predictors with block-wise correlation. Every 10 consecutive variables are highly correlated ($\rho=0.95$) within each block, while blocks are independent. Only 4 variables have nonzero effects on the log rate, located in different correlation blocks.
...
linear_pred <- drop(1.5 + X %*% theta_true)  # with intercept for reasonable counts
...
We allow up to $L=10$ single effects and use the Poisson family for count regression. The method is robust to larger $L$ values with minimal overfitting risk.
